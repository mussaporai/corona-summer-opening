...
// V3: reunião ganha status de conclusão, participantes, pauta estruturada e pendências.
function migrateMeetingsV3(state) {
  let changed = false;
  if (!Array.isArray(state.meetings)) { state.meetings = []; changed = true; }
  state.meetings.forEach(m => {
    if (typeof m.done !== "boolean") { m.done = false; changed = true; }
    if (!Array.isArray(m.participantes)) { m.participantes = []; changed = true; }
    if (!Array.isArray(m.pauta)) { m.pauta = []; changed = true; }
    if (!Array.isArray(m.actions)) { m.actions = []; changed = true; }
  });
  return changed;
}
