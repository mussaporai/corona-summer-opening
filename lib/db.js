...
async function kvGetWithVersion(key) {
  const { data, error } = await db().from("kv_store").select("value, updated_at").eq("key", key).maybeSingle();
  if (error) throw error;
  return data ? { value: data.value, version: data.updated_at } : null;
}

async function kvSetIfUnchanged(key, value, expectedVersion) {
  const { data, error } = await db().from("kv_store")
...
module.exports = { kvGet, kvSet, kvGetWithVersion, kvSetIfUnchanged };
