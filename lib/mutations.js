...
// Só http(s) — impede um link tipo "javascript:..." salvo na biblioteca de
// virar execução de script pra quem clicar em "Abrir link" depois.
...
const HANDLERS = {
  ...
  "add-file"(state, p, who) {
    ...
    const rawLink = (p && String(p.value || "").trim()) || "";
    if (type === "link" && rawLink && !isSafeHttpUrl(rawLink)) return;
    state.files.push({
      id: uid(), title, category, type, frente, link: rawLink,
  ...
  "toggle-file-principal"(state, p, who) {
    const f = state.files.find(x => x.id === p.id);
    if (!f) return;
    f.principal = !!p.value;
    log(state, who, `${f.principal ? "marcou" : "desmarcou"} "${(f.title||"").slice(0,60)}" como arquivo principal`);
  },
  ...
  "edit-file-field"(state, p, who) {
    ...
    if (field === "link" && f.type === "link" && val && !isSafeHttpUrl(val)) return;
    if (val === (f[field] || "")) return;
    f[field] = val;
  ...
  "replace-file-content"(state, p, who) {
    ...
    const val = String(p.value || "").trim();
    if (!val) return;
    if (f.type === "link" && !isSafeHttpUrl(val)) return;
    f.link = val;
