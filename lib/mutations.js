...
const HANDLERS = {
  ...
  "add-fornecedor"(state, p, who) {
    const found = findItemAndCat(state, p.itemId);
    if (!found) return;
    found.item.fornecedores.push({ id: uid(), supplier: "", contact: "", phone: "", proposalLink: "", filesLink: "", notes: "", accepted: null });
    log(state, who, `adicionou um fornecedor em ${found.item.code}`);
  },
  "remove-fornecedor"(state, p, who) {
    const found = findItemAndCat(state, p.itemId);
    if (!found) return;
    const idx = found.item.fornecedores.findIndex(f => f.id === p.fornecedorId);
    if (idx === -1) return;
    found.item.fornecedores.splice(idx, 1);
    log(state, who, `removeu um fornecedor de ${found.item.code}`);
  },
  "edit-fornecedor-field"(state, p, who) {
    const found = findItemAndCat(state, p.itemId);
    if (!found) return;
    const f = found.item.fornecedores.find(f => f.id === p.fornecedorId);
    if (!f) return;
    const field = p.field;
    ...
    f[field] = val;
    log(state, who, `editou "${FORNECEDOR_FIELD_LABELS[field]}" de um fornecedor em ${found.item.code}`);
  },
  "set-fornecedor-accepted"(state, p, who) {
    ...
    f.accepted = value;
    log(state, who, `marcou fornecedor de ${found.item.code} como ${value === null ? "pendente" : (value ? "aceito" : "recusado")}`);
  },
