...
const HANDLERS = {
  ...
  "add-meeting"(state, p, who) {
    state.meetings.push({ id: uid(), theme: "Nova reunião — clique para editar", when: "", who: "", link: "", notes: "", done: false, participantes: [], pauta: [], actions: [] });
  ...
  "toggle-meeting-done"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.id);
    if (!m) return;
    m.done = !!p.done;
    log(state, who, `marcou reunião "${(m.theme||"").slice(0,60)}" como ${m.done ? "realizada" : "não realizada"}`);
  },
  "duplicate-meeting"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.id);
    if (!m) return;
    const clone = {
      id: uid(), theme: m.theme, when: "", who: m.who, link: m.link, notes: "", done: false,
      participantes: [...(m.participantes||[])],
      pauta: (m.pauta||[]).map(pt => ({ id: uid(), text: pt.text, decision: "" })),
      actions: [],
    };
    state.meetings.push(clone);
    log(state, who, `duplicou a reunião "${(m.theme||"").slice(0,60)}"`);
  },
  ...
  "add-agenda-item"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.id);
    if (!m) return;
    if (!Array.isArray(m.pauta)) m.pauta = [];
    m.pauta.push({ id: uid(), text: "", decision: "" });
    log(state, who, `adicionou um ponto de pauta a "${(m.theme||"").slice(0,60)}"`);
  },
  "rm-agenda-item"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.meetingId);
    if (!m || !Array.isArray(m.pauta)) return;
    m.pauta = m.pauta.filter(pt => pt.id !== p.itemId);
    log(state, who, `removeu um ponto de pauta de "${(m.theme||"").slice(0,60)}"`);
  },
  "edit-agenda-item"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.meetingId);
    if (!m || !Array.isArray(m.pauta)) return;
    const pt = m.pauta.find(x => x.id === p.itemId);
    if (!pt || !["text", "decision"].includes(p.field)) return;
    const val = String(p.value || "").trim();
    if (val === (pt[p.field] || "")) return;
    pt[p.field] = val;
    log(state, who, `editou um ponto de pauta de "${(m.theme||"").slice(0,60)}"`);
  },
  "agenda-to-action"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.meetingId);
    if (!m || !Array.isArray(m.pauta)) return;
    const pt = m.pauta.find(x => x.id === p.itemId);
    if (!pt) return;
    if (!Array.isArray(m.actions)) m.actions = [];
    m.actions.push({ id: uid(), text: pt.text || "Pendência da reunião", responsavel: "", prazo: "", done: false });
    log(state, who, `transformou um ponto de pauta em pendência em "${(m.theme||"").slice(0,60)}"`);
  },
  "add-action"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.id);
    if (!m) return;
    if (!Array.isArray(m.actions)) m.actions = [];
    m.actions.push({ id: uid(), text: "", responsavel: "", prazo: "", done: false });
    log(state, who, `adicionou uma pendência a "${(m.theme||"").slice(0,60)}"`);
  },
  "rm-action"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.meetingId);
    if (!m || !Array.isArray(m.actions)) return;
    m.actions = m.actions.filter(a => a.id !== p.itemId);
    log(state, who, `removeu uma pendência de "${(m.theme||"").slice(0,60)}"`);
  },
  "edit-action-field"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.meetingId);
    if (!m || !Array.isArray(m.actions)) return;
    const a = m.actions.find(x => x.id === p.itemId);
    if (!a || !["text", "responsavel", "prazo"].includes(p.field)) return;
    const val = String(p.value || "").trim();
    if (val === (a[p.field] || "")) return;
    a[p.field] = val;
    log(state, who, `editou uma pendência de "${(m.theme||"").slice(0,60)}"`);
  },
  "toggle-action-done"(state, p, who) {
    const m = state.meetings.find(x => x.id === p.meetingId);
    if (!m || !Array.isArray(m.actions)) return;
    const a = m.actions.find(x => x.id === p.itemId);
    if (!a) return;
    a.done = !!p.done;
    log(state, who, `${a.done ? "concluiu" : "reabriu"} uma pendência de "${(m.theme||"").slice(0,60)}"`);
  },
  "add-file"(state, p, who) {
    ...
    state.files.push({
      id: uid(), title, category, type,
      link: (p && String(p.value || "").trim()) || "",
      addedBy: who || "", createdAt: Date.now(), favorite: false
    });
    log(state, who, `adicionou ${type === "link" ? "um link" : "um arquivo"} à biblioteca: "${title.slice(0,60)}"`);
  },
  "rm-file"(state, p, who) {
