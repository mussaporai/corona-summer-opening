...
async function syncMeetingToGoogle(type, payload, state) {
  ...
  try {
    ...
    if (!auth.connected || !auth.refreshToken || !auth.calendarId) return;
    ...
    if (type === "rm-meeting") {
      if (existingEventId) {
        await deleteEvent(accessToken, calendarId, existingEventId);
    ...
    if (existingEventId) {
      await updateEvent(accessToken, calendarId, existingEventId, eventBody);
    } else {
